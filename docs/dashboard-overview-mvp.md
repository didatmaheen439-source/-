...
- 今日待办：聚合待审核、待发布、被驳回内容、待处理反馈、反馈超时、学习路径预校验阻断、学习路径驳回、发布失败、回滚失败和权限异常。
...
- 模块状态摘要：展示审核发布、题库内容、用户反馈、学习路径、系统审计和 AI 占位摘要。
...
工作台不维护静态正式数字，所有正式数值来自已有 mock 业务数据。
