...
- 模块状态摘要：展示审核发布、题库内容、用户反馈、学习路径、系统审计、AI 策略治理、写译批改和模考试卷摘要。
...
- 今日待办接入模考试卷待审核、待发布和预校验阻断。
- 模块摘要展示试卷总数、已发布、待审核、待发布和预校验异常数量。
- 最近处理记录可展示模考试卷创建、编辑、预校验、提交、发布、下架和回滚审计摘要。
- 快捷入口按 `mockExam.read` 权限返回 `/mock-exam/papers`，不向无权限角色暴露入口。
