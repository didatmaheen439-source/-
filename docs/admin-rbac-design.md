...
- 七类角色均保留 `dashboard.read`，用于登录后进入角色化工作台。
- `super_admin` 可见全量工作台区块、待办、风险和快捷入口，并可调用工作台风险处理接口。
- `content_operator` 只返回内容、审核发布和本人可处理范围内的待办，不返回系统、用户反馈等无权限入口。
- `teaching_reviewer` 返回审核发布、学习路径、题库内容和教研范围待办。
- `ai_operator` 返回 AI、审核发布和相关占位摘要，不返回系统配置入口。
- `customer_support` 返回用户反馈、反馈超时和敏感访问提醒，不返回内容编辑和审核发布写入口。
